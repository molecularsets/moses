...
class LatentGANTrainer(MosesTrainer):
    ...
    def _train(self, model, train_loader, val_loader=None, logger=None):
        ...
        for epoch in range(self.config.train_epochs):
            ...
            # Putting the generator and discriminator weights onto the model instance for checkpoint purposes
            # The parameters of the model are the weights used during the sampling stage later.
    ...
    def fit(self, 
        ...
        if self.config.heteroencoder_version == 'new':
            ...
            encoder_checkpoint_path = '{}/moses/latentgan/heteroencoder_models/checkpoints/'.format(currentDirectory)
            # Convert all SMILES to binary RDKit mols to be compatible with the heteroencoder
            heteroencoder_mols = [Chem.rdchem.Mol.ToBinary(Chem.MolFromSmiles(smiles)) for smiles in train_data]
            # Dataset information     
