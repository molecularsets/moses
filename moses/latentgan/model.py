...
class LatentGAN(nn.Module):
    ...
    def sample(self, n_batch, max_length =100):
        if self.model_loaded == False:
            ...
            if n_batch <= 256:
                print('Batch size of {} detected. Decoding performs poorly when Batch size != 256. \
                 Setting batch size to 256'.format(n_batch))
        # Sampling performs very poorly on default sampling batch parameters. This takes care of the default scenario.
        if n_batch == 32:
            n_batch=256
        ...
        if self.new_batch_size != n_batch:
            # The batch decoder creates a new instance of the decoder every time a new batch size is given, e.g. for the
