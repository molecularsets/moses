    ...
    def _pretrain_epoch(self, model, tqdm_data, criterion, optimizer=None):
        ...
        for i, (encoder_inputs,
            ...
            decoder_outputs = torch.cat(
                [
                    t[:l]
                    for t, l in zip(decoder_outputs,
            ...
            decoder_targets = torch.cat(
                [
                    t[:l]
                    for t, l in zip(*decoder_targets)
                ], dim=0
            )
    ...
    def get_collate_fn(self, model):
        ...
        def collate(data):
            ...
            lengths = torch.tensor(
                [len(t) for t in tensors], dtype=torch.long, device=device
            )

            encoder_inputs = pad_sequence(tensors, batch_first=True,
