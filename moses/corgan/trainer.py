...
class ORGANTrainer:
    ...
    def _pretrain_generator_epoch(self, model, tqdm_data, criterion, optimizer=None):
        ...
        for i, (batch, c) in enumerate(tqdm_data):
            (prevs, nexts, lens) = [x.to(model.device) for x in batch]
            # condition
            c = c.to(model.device)
            outputs, _, _ = model.generator_forward(prevs, lens, c)
    ...
    def _pretrain_generator(self, model, train_data, fps_data, val_data=None):
        def collate(tensors):
        ...
        for epoch in range(self.config.generator_pretrain_epochs):
            # training data
            train_data = zip(train_loader, fps_loader)
            shuffle(train_data)
            tqdm_data = tqdm(train_data, desc='Generator training (epoch #{})'.format(epoch))
    ...
    def _pretrain_discriminator_epoch(self, model, tqdm_data, criterion, optimizer=None):
        ...
        for i, (inputs_from_data, c) in enumerate(tqdm_data):
            # condition
            c = c.to(model.device)

            inputs_from_data = inputs_from_data.to(model.device)
            inputs_from_model, _ = model.sample_tensor(c, self.config.n_batch, self.config.max_length)
            targets = torch.zeros(self.config.n_batch, 1, device=model.device)
            outputs = model.discriminator_forward(inputs_from_model, c)
            loss = criterion(outputs, targets) / 2
            targets = torch.ones(inputs_from_data.shape[0], 1, device=model.device)
            outputs = model.discriminator_forward(inputs_from_data, c)
            loss += criterion(outputs, targets) / 2
    ...
    def _pretrain_discriminator(self, model, train_data, fps_data, val_data=None):
        def collate(data):
        ...
        for epoch in range(self.config.discriminator_pretrain_epochs):
            ...
            train_data = zip(train_loader, fps_loader)
            shuffle(train_data)
            tqdm_data = tqdm(train_data, desc='Discriminator training (epoch #{})'.format(epoch))
    ...
    def _train_policy_gradient(self, model, train_data, fps_data):
        def collate(data):
            data.sort(key=lambda x: len(x), reverse=True)
            tensors = data
            inputs = pad_sequence(tensors, batch_first=True, padding_value=model.vocabulary.pad)
        ...
        train_loader = DataLoader(SmilesDataset(train_data, transform=model.string2tensor),
                                  batch_size=self.config.n_batch, shuffle=True, collate_fn=collate, drop_last=True)
        
        fps_loader = DataLoader(fps_data, batch_size=self.config.n_batch, shuffle=True, drop_last=True)
        ...
        for i in pg_iters:
            ...
            for _ in range(self.config.generator_updates):
                ...
                sequences, rewards, lengths = model.rollout(
                    c, self.config.n_batch, self.config.rollouts, self.config.max_length)
                model.train()
                ...
                generator_outputs, lengths, _ = model.generator_forward(sequences[:, :-1], lengths - 1, c)
            ...
            for _ in range(self.config.discriminator_updates):
                ...
                sampled_batches = [model.sample_tensor(c, self.config.n_batch, self.config.max_length)[0]
                ...
                for _ in range(self.config.discriminator_epochs):
                    ...
                    for inputs_from_model, inputs_from_data in zip(sampled_batches, train_loader):
                        ...
                        discriminator_outputs = model.discriminator_forward(inputs_from_model, c)
                        ...
                        discriminator_outputs = model.discriminator_forward(inputs_from_data, c)
                        discriminator_loss += discriminator_criterion(discriminator_outputs, discriminator_targets) / 2
