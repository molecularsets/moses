...
def get_trainer(model):
    mapper = {'aae': aae_train,
              'char_rnn': char_rnn_train,
              'junction_tree': junction_tree_train,
              'organ': organ_train,
              'vae': vae_train}
...
def get_sampler(model):
    mapper = {'aae': aae_sample,
              'char_rnn': char_rnn_sample,
              'junction_tree': junction_tree_sample,
              'organ': organ_sample,
              'vae': vae_sample}
...
def sample_from_model(config, model):
    ...
    sampler_config = sampler_parser.parse_args(['--device', config.device,
                                                '--model_load', os.path.join(config.checkpoint_dir, model + '_model.pt'),
                                                '--config_load', os.path.join(config.checkpoint_dir, model + '_config.pt'),
                                                '--vocab_load', os.path.join(config.checkpoint_dir, model + '_vocab.pt'),
