...
if __name__ == '__main__':
    ...
    model = CharRNN(vocab, new_config.hidden, new_config.num_layers, device)
    model.load_state_dict(torch.load(new_config.model_load))
    model = model.to(device=device)

    gen_smiles = model.sample_smiles(new_config.n_samples, new_config.max_len)
    gen_smiles = vocab.reverse(gen_smiles)
