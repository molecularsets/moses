...
from mnist4molecules.trainer import Trainer
from vae.misc import CosineAnnealingLRWithRestart, KLAnnealer, Logger
...
class VAETrainer(Trainer):
    def fit(self, model, dataloader, corpus):
        ...
        n_iter, n_last = len(dataloader), self.config.n_last
        ...
        for epoch in T:
            ...
            # Log
            sent = corpus.reverse(self.sample(model, 1))[0]
            elog.append({
    ...
    def sample(self, model, n):
        model.eval()
        return model.sample(n, self.config.n_len)[-1]
