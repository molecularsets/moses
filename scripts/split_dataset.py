...
def get_parser():
    ...
    parser.add_argument(
        "--dir", type=str, default="./data", help="Directory for splitted dataset"
    )
    parser.add_argument(
        "--no_subset",
        action="store_true",
    ...
    parser.add_argument(
        "--train_size", type=int, default=250000, help="Size of training dataset"
    )
    parser.add_argument(
        "--test_size", type=int, default=10000, help="Size of testing dataset"
    )
...
def main(config):
    ...
    if not os.path.exists(dataset_path):
        raise ValueError(
            f"Missing dataset_v1.csv in {config.dir}; "
    ...
    if config.precompute:
        ...
        np.savez(
            os.path.join(config.dir, "test_scaffolds_stats.npz"), stats=test_sf_stats
        )
