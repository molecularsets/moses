...
def get_parser():
    ...
    parser.add_argument("--reconstruct",
                            ...
                            help="Whether to reconstruct validation/test data",
    )
    return parser
...
def sample_from_model(config, model,val_path):
    '''
...
def main(config):
    ...
    val_path = os.path.join(config.data_dir, "train5k.csv")
    ...
    for model in models:
        ...
        sample_from_model(config, model,val_path)
    metrics = []
    for model in models:
