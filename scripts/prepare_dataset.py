...
def get_parser():
    ...
    parser.add_argument(
        "--n_jobs", type=int, default=1, help="number of processes to use"
    )
    parser.add_argument(
        "--keep_ids", action="store_true", help="Keep ZINC ids in the final csv file"
    )
...
def filter_lines(lines, n_jobs):
    ...
    with Pool(n_jobs) as pool:
        dataset = [
            x
            for x in tqdm.tqdm(
                pool.imap_unordered(process_molecule, lines),
                total=len(lines),
                miniters=1000,
            )
            if x is not None
